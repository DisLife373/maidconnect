    ...
    <div className={name ? "h-full profilebox-wrapper rounded-3xl border-4 border-dark-pink hover:bg-light-pink/25 transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-100 duration-300" : "h-full profilebox-wrapper rounded-3xl border-4 border-background-color hover:bg-light-green transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-100 duration-300"}>
      <section className="profilebox-container p-3 grid grid-cols-4 content-center gap-3 h-full" id={name && "cancel"}>
        ...
        <div className="profilebox-content col-span-3 grid gap-y-2 ">
          ...
          >
            ...
            <section className="job-date">
              <section>
                ...
                >วันที่ :</b>
                <span className="text-sx sm:text-sm md:text-base">{result}</span>
              </section>
              <section>
                <b>เวลา :</b>
            ...
            {!maidmain ?
              <section className="job-chips flex flex-wrap gap-x-2 gap-y-1">
                {user.jobs?.slice(0, 9).map((job, job_index) => (
                  <span key={job_index} className="text-ss sm:text-sx md:text-sm bg-light-pink px-2 rounded-full">{job.job_name}</span>
                ))}
                {user.jobs?.length > 9 && <span className="text-ss sm:text-sx md:text-sm bg-light-pink px-2 rounded-full"> more... </span>}
              </section>
              :
              <section className="job-chips flex flex-wrap gap-x-2 gap-y-1">
          ...
          {canClick && buttonName !== "" ? (
            <footer className={name ? "justify-self-end grid justify-items-end": clickCancel && clickConfirm ? "justify-self-end grid grid-cols-2 justify-items-end gap-x-2" : "justify-self-end grid justify-items-end"}>
              {name && (
              ...
              {clickConfirm && (
                <button onClick={clickConfirm} className="text-white text-sx sm:text-sm md:text-base bg-green-darkness px-3 py-1 rounded-full hover:bg-background-color active:bg-green-darkness transition duration-500"> {buttonName} </button>
              )}
              {clickCancel && (
                <button onClick={clickCancel} className="text-green-darkness text-sx sm:text-sm md:text-base px-3 py-1 rounded-full border-2 border-green-darkness hover:border-background-color hover:bg-background-color hover:text-white active:bg-green-darkness">
                  {/* {" "} */}
                  ปฏิเสธ
                  {/* {" "} */}
                </button>
            ...
            <footer className="footer-nobutton justify-self-end">
              <p style={{ margin: "12px 0 0 0" }} className="text-sx sm:text-sm md:text-base"> {buttonName} </p>
            </footer>
          )}
        </div>
      </section>
