...
function MaidMain() {
  ...
  return (
    <main className="main-maid grid grid-cols-3">
      <header className="calendar grid content-center bg-light-green h-screen px-10 py-5">
        <section className="">
          <DayPicker
            ...
            classNames={{
              caption_label:
                "text-dark-green text-basexl sm:text-xl md:text-2xl",
              nav_button:
                "text-dark-green bg-transparent hover:text-white p-3 transition-colors duration-300",
              head_row:
                "text-dark-green text-base sm:text-basexl md:text-xl border-t-4 border-b-2 border-green-darkness",
              // tbody: "justify-self-end",
              day_selected:
                "text-white bg-dark-pink rounded-full transition duration-20",
      ...
      <main
        className={
          "profile-box px-10 py-5 flex flex-wrap flex-col-reverse overscroll-contain overflow-y-auto col-span-2 w-ful h-fit gap-y-2"
        }
      >
        {invoices.map((invoice, invoiceid) => (
          <section key={invoiceid} className="grid grid-cols-2 max-h-64">
            {invoice.user_id && (
              <ProfileBox
                user={invoice}
